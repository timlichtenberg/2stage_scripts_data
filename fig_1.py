...
ax1.set_xticks( [0.5, 1, 2, 3, 5, 10, 20] )
ax1.set_xticklabels( ["0.5", "1", "2", "3", "5", "10", "20"], fontsize=fsize-2 )
...
# Snowline
ax1.plot(q.rsnow[26:]/au,q.time[26:]/year,color="black",linestyle='-', lw=2.5, zorder=1)
...
yloc_colorlegend = +0.10

# Res I
cax = inset_axes(ax1, width='30%', height='2%', loc='lower right', bbox_to_anchor=(xloc_colorlegend, yloc_colorlegend, 1, 1), bbox_transform=ax1.transAxes, borderpad=0)
...
# Res II
cbaxes2 = inset_axes(ax1, width="30%", height="2%", loc='lower right', bbox_to_anchor=(xloc_colorlegend, yloc_colorlegend+0.03, 1, 1), bbox_transform=ax1.transAxes, borderpad=0.0)
clb2 = plt.colorbar(cont_resII, cax=cbaxes2, ticks=v_legend, orientation="horizontal")
clb2.ax.tick_params(labelsize=12, labelrotation=45)
clb2.ax.set_xticklabels(v_nat_mag_str)
cbaxes2.xaxis.set_ticks_position('top')

clb2.ax.text(-0.5, -0.5, 'Planetesimal formation\nrate, '+r'$\mathrm{d}\Sigma_{\rm{plts}}/\mathrm{d}t$', fontsize=fsize-3, rotation=0, va='center', ha="center") 
clb2.ax.text(0.55, 4.6, r'log $M_\mathrm{Earth}\ \mathrm{au}^{-2}\ \mathrm{Myr}^{-1}$', fontsize=fsize-5, rotation=0, va='center', ha="center") 
clb.ax.text(0.55, -3.4, r'log $\mathrm{g}\ \mathrm{cm}^{-2}\ \mathrm{s}^{-1}$', fontsize=fsize-5, rotation=0, va='center', ha="center") 
...
color_mig = qred_dark
ax1.text(0.44, 0.58, 'Terrestrial planet\nmigration corridor', color=color_mig, rotation=0, ha="center", va="center", fontsize=fsize-3, transform=ax1.transAxes)
ax1.text(0.38, 0.935, r'$0.1 \, M_{\mathrm{Earth}}$', color=color_mig, size=fsize-7, rotation=-40, va='center', ha='center', transform=ax1.transAxes)
ax1.text(0.38, 0.777, r'$0.2 \, M_{\mathrm{Earth}}$', color=color_mig, size=fsize-7, rotation=-33, va='center', ha='center', transform=ax1.transAxes)
ax1.text(0.39, 0.433, r'$0.01 \, M_{\mathrm{Earth}}$', color=color_mig, size=fsize-7, rotation=-46, va='center', ha='center', transform=ax1.transAxes)
ax1.text(0.38, 0.37, r'$0.1 \, M_{\mathrm{Earth}}$', color=color_mig, size=fsize-7, rotation=-38, va='center', ha='center', transform=ax1.transAxes)
...
ax1.text(x_text, 0.745, 'Class II stage', color=qgray_dark, size=fsize, rotation=90, va='center', ha='left', fontsize=fsize-5, transform=ax1.transAxes)
...
ax1.text(x_text, 0.236, 'Infall stage', color=qgray_dark, size=fsize, rotation=90, va='center', ha='left', fontsize=fsize-5, transform=ax1.transAxes)
...
########## RESERVOIRS
ax1.text(0.88, 0.17e+6, r'H$_2$O'+'\nsnowline', color="k", size=fsize, rotation=0, va='center', ha='center', fontsize=fsize-3)
