...
cbar_ticks = [ np.min(zi_perco_frac), 0.2, 0.4, 0.6, 0.8, np.max(zi_perco_frac) ]

no_color_fields = 255
cbar_label = r"Potential percolative core formation, $f_\mathrm{perco, final}$ (vol%)"
...
tform_B = np.asarray([ 0.3, 0.72, 1.3 ])
print(rad_B, tform_B)
...
T3_text = ax1.text(93, 1.28, 'T3', color=qmagenta, rotation=0, fontsize=fsize+2, ha="right", va="top", zorder=10, bbox=dict(fc='white', ec="white", alpha=0.0, pad=0.1, boxstyle='round'))
...
for text in [ R3_text, T1_text, T2_text, R1_text, R2_text, T3_text ]:
    text.set_path_effects([path_effects.Stroke(linewidth=1, foreground='white'),
                       path_effects.Normal()])

# for text in [ R1_text, R2_text, T3_text ]:
#     text.set_path_effects([path_effects.Stroke(linewidth=1, foreground='white'),
#                        path_effects.Normal()])
...
plt.savefig(fig_dir+"fig_s6a"+".pdf", bbox_inches='tight')
